...
class Main inherits IO {
	...
	main() : SELF_TYPE {

		{
			...
			strAttr@String.concat(" ");


			strAttr.concat("Roshani");
			...
			if strAttr = "int" then new Int else new String fi;

			let i : Int <- 0 in while i < 10 loop {
				out_int(i);
				out_string("\n");
				i <- i + 1;
			} pool;
			...
			(* isvoid, not, bool_const (least common ancestor of two SELF_TYPEs) *)
			if isvoid while not true loop 0 pool then
				out_string("Cool Manual is truth-teller")
			else
				out_string("Cool Manual is liar")
			fi;
			...
			(* least common ancestor of SELF_TYPE and child *)
			if false then
				self
			else
				new MainC
